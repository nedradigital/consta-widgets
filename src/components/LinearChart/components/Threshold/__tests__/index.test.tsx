import { getFillPoints, isStraightLine } from '../'
...
describe('Линейный график / линии порога', () => {
  ...
  describe('isStraightLine', () => {
    it('Определяет, что горизонтальная линия ровная', () => {
      expect(isStraightLine([{ x: 0, y: 1 }, { x: 1, y: 1 }, { x: 2, y: 1 }], true)).toEqual(true)
    })

    it('Определяет, что горизонтальная линия неровная', () => {
      expect(isStraightLine([{ x: 0, y: 1 }, { x: 1, y: 2 }, { x: 2, y: 1 }], true)).toEqual(false)
    })

    it('Определяет, что вертикальная линия ровная', () => {
      expect(isStraightLine([{ x: 1, y: 0 }, { x: 1, y: 1 }, { x: 1, y: 2 }], false)).toEqual(true)
    })

    it('Определяет, что вертикальная линия неровная', () => {
      expect(isStraightLine([{ x: 1, y: 0 }, { x: 0, y: 1 }, { x: 1, y: 2 }], false)).toEqual(false)
    })
  })
})
