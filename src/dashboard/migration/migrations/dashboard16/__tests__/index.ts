import { Dashboard16, migration16, widgetIdsByType as currentWidgetIdsByType } from '../'
import {
...
describe('migration16', () => {
  it('повышает версию', () => {
    ...
    const result: Dashboard16.State = {
      version: 16,
      boxes: [],
      config: {
        Box0: [
    ...
    expect(migration16.up(source)).toEqual(result)
  })

  it('понижает версию', () => {
    const source: Dashboard16.State = {
      version: 16,
      boxes: [],
      config: {
        Box0: [
    ...
    expect(migration16.down(source)).toEqual(result)
  })
})
